...
ggplot(suma_presion_ENP_3, aes(x = interaction(ZONA, ENP), y = Proporción, fill = Categoria)) +
  ...
  theme(plot.title = element_text(hjust = 0.5, size = 13,face="bold"),  # Centrar y reducir el tamaño del título
...
suma_presion_cost_int_3 <- datos %>%
  ...
  pivot_longer(cols = c(num_positivos, num_negativos,sin_cambios),  # Transformar a formato largo para apilamiento
               names_to = "Categoria",
               values_to = "Valor") %>%
  mutate(Proporción = Valor / total_puntos) %>%  # Calcular la proporción sobre el total
  mutate(Categoria = factor(Categoria, levels = c("num_positivos", "num_negativos","sin_cambios")))  # Definir el orden de las categorías
...
modelo_presion <- glmer(PRESION_BIN~ENP+GRUPO_TIPO+ENP*ZONA+GRUPO_TIPO*ZONA +(1|COD_IHA),family=binomial, datos)
Anova(modelo_presion)

datos$MUCVA1984_GENERAL <- as.factor(datos$MUCVA1984_GENERAL)
datos$CAMBIO <- as.numeric(datos$CAMBIO)
modelo_glmer_USOS<- glmer(CAMBIO ~ MUCVA1984_GENERAL +(1|COD_IHA),family=binomial, datos)
summary(modelo_glmer_USOS)
Anova(modelo_glmer_USOS)
